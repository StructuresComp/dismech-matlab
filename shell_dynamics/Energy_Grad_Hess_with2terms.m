...
% E_with_stiff = stiff*(nu*E + (1-nu)*E2).*A^2 ;
%% Gradient of Energy
...
for i=1:3
    for j=1:3
        ...
        del_E2_del_pi = del_E2_del_pi - c(i)*c(j) * ((s(i)*xi(i) - f(i)) .* delfi_by_delpk(tau_0(:,j), t_i, unit_norm, A) + ...
            (s(j)*xi(j) - f(j)) .* delfi_by_delpk(tau_0(:,i), t_i, unit_norm, A)) .* (norm(t(:,i))^2*norm(t(:,j))^2); % (l(i)^2*l(j)^2)
...
% gradE_with_stiff = stiff.*(nu.*gradE + (1-nu).*gradE2).*A^2;
%% Hessian of Energy
...
%             ddel_E_by_ps(:,:,k) = ddel_E_by_ps(:,:,k) - (c(i)*c(j)) * (dot(t(:,i),t(:,j))^2) .* (...
%             (s(i)*xi(i) - f(i))*ddel_fi_del_p_k1_p_k2(vi,vj,vk,tau_0(:,j),unit_norm,A,char_k1,char_k2) - ...
%             transpose(delfi_by_delpk(tau_0(:,j), t(:,num_k2), unit_norm, A)) * delfi_by_delpk(tau_0(:,i), t(:,num_k1), unit_norm, A)...
%             + (s(j)*xi(j) - f(j))*ddel_fi_del_p_k1_p_k2(vi,vj,vk,tau_0(:,i),unit_norm,A,char_k1,char_k2) - ...
%             transpose(delfi_by_delpk(tau_0(:,i), t(:,num_k2), unit_norm, A)) * delfi_by_delpk(tau_0(:,j), t(:,num_k1), unit_norm, A) ) ;
...
% hessE_with_stiff = stiff.*(nu*hessE + (1-nu)*hessE2).*A^2;
end