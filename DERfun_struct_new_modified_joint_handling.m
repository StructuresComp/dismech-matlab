function [MultiRod, stretch_springs, bend_twist_springs, hinge_springs] = ...
    DERfun_struct_new_modified_joint_handling (MultiRod, stretch_springs, bend_twist_springs, hinge_springs)
...
%     compute the location of the pseudo node
    q(mapNodetoDOF(n_nodes)) = 0.5.*(q(mapNodetoDOF(3)) + q(mapNodetoDOF(5)));
     ...
     [m1, m2] = computeMaterialDirectors(a1_iter,a2_iter,theta); 
      ...
      % material frame for pseudo joint is computed as the normal of the
      % shell face
...
theta_0 = signedAngle(a1_iter(end,:), m1(end,:), (q(mapNodetoDOF(n_nodes))-q(mapNodetoDOF(2)) ) );
...
% 
     ...
     offset = (q(22)-theta_0)*0.5*(1/norm(q(mapNodetoDOF(6)) - q(mapNodetoDOF(2))))...
